...
<div class="space-y-8">
	...
	<div class="bg-white rounded-2xl shadow-lg p-6 border border-[#122555]/10">
		<div class="flex items-center gap-4">
			...
			{/if}
			<div>
				<h1 class="text-2xl font-bold text-[#122555]">{user?.name || 'Utilisateur'}</h1>
				<p class="text-[#122555]/60">{user?.email}</p>
			</div>
		</div>
	</div>
	...
	<div class="grid grid-cols-2 md:grid-cols-4 gap-4">
		...
		<div class="bg-white rounded-xl shadow p-4 border border-[#122555]/10">
			<p class="text-[#122555]/60 text-sm flex items-center gap-1">
				<BuildingIcon class="w-4 h-4" /> Meilleur Orga
			</p>
			<p
	...
	<div class="grid md:grid-cols-2 gap-8">
		...
		<div class="bg-white rounded-2xl shadow-lg p-6 border border-[#122555]/10">
			<h2 class="text-xl font-bold text-[#122555] mb-4 flex items-center gap-2">
				<BarChartIcon class="w-5 h-5" /> Réussite par catégorie
			</h2>
					...
					{#each Object.entries(stats.categoryStats) as [category, catData]}
						...
						<div>
							<div class="flex justify-between mb-1">
								...
								<span class="text-sm font-medium {getScoreColor(data.percentage)}"
									>{data.percentage}%</span
								>
							</div>
							<div class="w-full bg-[#122555]/10 rounded-full h-2.5">
								<div
									class="h-2.5 rounded-full transition-all duration-500 {data.percentage >= 80
										? 'bg-green-500'
										: data.percentage >= 60
											? 'bg-yellow-500'
											: 'bg-red-500'}"
			...
			{#if chartDataOrga.length > 0 || chartDataTreso.length > 0}
				...
				<div class="flex justify-center gap-6 mt-4">
					<div class="flex items-center gap-2">
						...
						<span class="text-sm text-[#122555]">Orga ({chartDataOrga.length})</span>
					</div>
					<div class="flex items-center gap-2">
				...
				<table class="w-full">
					<thead>
						<tr class="border-b border-[#122555]/10">
							...
							<th class="text-center py-3 px-2 text-[#122555]/60 text-sm font-medium">Questions</th>
