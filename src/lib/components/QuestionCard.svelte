<script lang="ts">
	...
	import type { QuizAnswer, Question, AnswerOption } from '$lib/types.js';

	let { onComplete } = $props<{ onComplete: () => void }>();

	let quiz = $state<Question[]>([]);
	let questionIndex = $state(0);
	let answers = $state<QuizAnswer[]>([]);
	let selectedOptionText = $state<string | null>(null);
	...
	const correctOption = $derived(currentQuestion?.answerOptions.find((o) => o.isCorrect));
	const selectedOption = $derived(
		currentQuestion?.answerOptions.find((o) => o.text === selectedOptionText)
	);
...
{#if currentQuestion}
	<div>
		<div class="bg-white rounded-2xl shadow-xl overflow-hidden">
			<!-- Progress Bar -->
			<div class="bg-[#122555]/10 h-2">
				<div
					class="bg-[#122555] h-full transition-all duration-300 ease-out"
			...
			<!-- Question Header -->
			<div
				class="px-6 py-4 bg-gradient-to-r from-[#122555]/5 to-[#122555]/10 border-b border-[#122555]/10"
			>
				<div class="flex justify-between items-center">
					<span class="text-sm font-medium text-[#122555]/70">
						Question {questionIndex + 1} sur {quiz.length}
					</span>
					<span class="px-3 py-1 rounded-full text-xs font-medium bg-[#122555]/10 text-[#122555]">
			...
			<div class="p-8 min-h-[400px]">
				...
				<div class="space-y-3">
					{#each currentQuestion.answerOptions as option, index}
						<button
							...
							disabled={isAnswered}
							class="w-full p-4 text-left rounded-lg border-2 transition-all duration-200 transform hover:scale-[1.02] {isAnswered
								? option.isCorrect
									? 'border-green-500 bg-green-50 text-green-800'
									: option.text === selectedOptionText
										? 'border-red-500 bg-red-50 text-red-800'
										: 'border-gray-200 bg-gray-50 text-gray-500'
								: 'border-gray-200 hover:border-blue-400 hover:bg-blue-50'} {isAnswered
								? 'cursor-not-allowed'
								: 'cursor-pointer'}"
						>
							<div class="flex items-center">
								<span
									class="w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm font-semibold mr-4 {isAnswered
										? option.isCorrect
								...
								{:else if isAnswered && option.text === selectedOptionText && !option.isCorrect}
				...
				{#if showFeedback}
					<div
						class="mt-6 p-4 rounded-lg {selectedOption?.isCorrect
					...
					>
						<div class="flex items-center justify-between mb-2">
							...
							<div class="flex gap-2">
								<button
									...
									class="px-3 py-1.5 text-sm bg-white border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-700 flex items-center gap-2 transition-colors"
								>
									<span>💡</span>
									{showRationale ? 'Masquer explication' : 'Voir explication'}
								</button>
								{#if canSkip}
									<button
										onclick={nextQuestion}
										class="px-4 py-2 bg-[#122555] hover:bg-[#0d1a3d] text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-2"
									>
										Suivant
										<div class="flex items-center gap-1">
											<span
												class="text-xs bg-[#122555]/70 px-1.5 py-0.5 rounded border border-white/20"
											>
												<svg class="w-3 h-3 inline fill-current" viewBox="0 0 350.439 350.439">
													<path
														d="M312.856,36.464H202.507c-4.87,0-8.83,3.961-8.83,8.832v138.422H52.371c-4.87,0-8.832,3.973-8.832,8.843v108.023
						...
						{#if !selectedOption?.isCorrect}
							<p class="text-sm text-red-700 mb-2">
								La bonne réponse est : <strong>{correctOption?.text}</strong>
							</p>
						{/if}

						{#if showRationale}
							<div class="mt-4 pt-4 border-t border-gray-200">
								<h4 class="text-sm font-semibold text-gray-700 mb-1">Explication :</h4>
